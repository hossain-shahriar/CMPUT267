...
student = (name="Mohammad Shahriar Hossain", email="mhossai6@ualberta.ca", ccid="mhossai6", idnumber=1724709)
...
# ╔═╡ dfef12e2-07ce-41ae-9e98-dadbbe424c10
# Generates samples, useful for 4b, 4c, 4d, 4e
function genSample(n::Int64, µ::Float64, σ::Float64)
	gd = GaussianDistribution(µ, σ)
	dataset = sample(gd, n)
end
...
let # 4b
	...
	print("Array of Sample average: ")
	println(M)
	print("\n")
	print("Sample Variance: ",var(M))
end
...
let # 4d, 4e
	...
	M = mean(D)
	V = var(D)
	print("Sample Average: ", M, "\n\n")
	#4d starts
	# 95% confidence interval: (mean(D)- 1.96*(var(D)/n), mean(D)+ 1.96*(var(D)/n))
	print("95% Confidence Interval assuming the samples are Gaussian: ")
	LBound = M - (1.96 * sqrt(V/n))
	UBound = M + (1.96 * sqrt(V/n))
	print("(",LBound,", ",UBound,")\n")
	#4d ends
	print("\n")
	#4e starts
	# 95% confidence interval: (mean(D)- sqrt(var(D)/(delta*n)), mean(D)+ sqrt(var(D)/(delta*n)))
	print("95% Confidence Interval without assuming the samples are Gaussian: ")
	δ = 0.05
	ε = sqrt(V/(δ*n))
	LBound = M - ε
	UBound = M + ε
